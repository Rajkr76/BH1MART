...
export default function ProductCard({ product }) {
  ...
  return (
    <div className={`group rounded-2xl border-4 border-amber-900 bg-amber-50 shadow-[6px_6px_0px_#78350f] overflow-hidden transition-transform duration-300 hover:scale-[1.03] hover:shadow-[8px_8px_0px_#78350f] ${isOutOfStock ? "opacity-60 grayscale" : ""}`}>
      {/* Dots bar */}
      ...
      <div className="p-5">
        ...
        {!isOutOfStock && (
          <>
            ...
            {qty > 5 && (
              <p className="text-[10px] font-bold text-yellow-700 mt-1">
                ⚠️ Qty &gt; 5 will redirect to WhatsApp for bulk order
              </p>
            )}
          </>
        )}
        ...
        {isOutOfStock ? (
          <div className="mt-4 w-full rounded-xl border-3 border-red-400 bg-red-100 py-2.5 text-sm font-black uppercase tracking-wider text-red-600 text-center">
            OUT OF STOCK
          </div>
        ) : (
          <button
